...
import { DELETE_MULTIPLE_PRODUCTS } from "@/graphql/mutations";
import toast from "react-hot-toast";

interface PaginatedMyProductsData {
  paginatedMyProducts: {
    products: Product[];
    total: number;
    hasMore?: boolean;
...
export default function SellerProducts() {
  ...
  // Mutation: typed result and variables
  const [deleteMultipleProducts, { loading: deleting }] = useMutation<
    { deleteMultipleProducts: boolean },
    { ids: string[] }
  >(DELETE_MULTIPLE_PRODUCTS, {
    update(cache: ApolloCache<any>, { data }) {
      if (!data?.deleteMultipleProducts) return;

      // Read existing cache (typed)
      const existing = cache.readQuery<PaginatedMyProductsData>({
        query: SELLER_PRODUCT_PAGINATION,
        variables: { limit, offset },
      });

      if (!existing?.paginatedMyProducts) return;

      const updatedProducts = existing.paginatedMyProducts.products.filter(
      ...
      cache.writeQuery({
        ...
        data: {
          paginatedMyProducts: {
            ...existing.paginatedMyProducts,
            products: updatedProducts,
            total: Math.max(0, existing.paginatedMyProducts.total - selectedProducts.length),
          },
        },
      });
    },
  ...
  // Tell Apollo the shape of the query result with the generic
  const { data, loading } = useQuery<PaginatedMyProductsData>(SELLER_PRODUCT_PAGINATION, {
    variables: { limit, offset },
  ...
  // Apply all filters (use Product type)
  const filteredProducts = products.filter((p: Product) => {
    ...
    const matchesStatus =
      statusFilter === "all" ||
      (statusFilter === "active" && (p as any).isActive) ||
      (statusFilter === "inactive" && !(p as any).isActive);

    const price = Number(p.price);
    const minPrice = priceRange.min ? Number(priceRange.min) : 0;
    const maxPrice = priceRange.max ? Number(priceRange.max) : Number.MAX_SAFE_INTEGER;
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // show immediate feedback while mutation runs
      toast.loading("Deleting products...", { id: "bulk-delete" });

      const res = await deleteMultipleProducts({
        variables: { ids: selectedProducts },
      });

      if (res.data?.deleteMultipleProducts) {
        toast.success("Products deleted successfully!", { id: "bulk-delete" });
        // clear selection AFTER cache update (mutation update handler updates cache)
        setSelectedProducts([]);
      } else {
        toast.error("Delete failed!", { id: "bulk-delete" });
      }
    } catch (error) {
      console.error("Deletion error:", error);
      toast.error("Something went wrong during deletion", { id: "bulk-delete" });
    }
  };
  ...
  if (sortOrder) {
    sortedProducts.sort((a: Product, b: Product) => (sortOrder === "asc" ? a.price - b.price : b.price - a.price));
  } else if (sortDate) {
    sortedProducts.sort((a: Product, b: Product) =>
      sortDate === "asc" ? Date.parse(a.createdAt || "") - Date.parse(b.createdAt || "") : Date.parse(b.createdAt || "") - Date.parse(a.createdAt || "")
    );
  }
  ...
  const toggleDateSort = () => {
    setSortDate((prev) => (prev === "asc" ? "desc" : prev === "desc" ? null : "asc"));
    setSortOrder(null);
  };
  ...
  const allSelected =
    sortedProducts.length > 0 && sortedProducts.every((product: Product) => selectedProducts.includes(product.id));

  const handleSelectAll = () => {
    if (allSelected) {
      setSelectedProducts([]);
    } else {
      const allIds = sortedProducts.map((product: Product) => product.id);
  ...
  return (
    <>
      ...
      <div className="flex min-h-screen mt-8">
        <aside
          className={`w-64 bg-gray-50 p-6 fixed md:static inset-y-0 left-0 z-40 transform ${
        ...
        <main className="flex-1 p-4 md:p-6 bg-white overflow-x-auto">
          <div className="flex flex-col md:flex-row justify-between items-start mb-6 md:mb-8 gap-4">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-gray-800 mb-2">Your Products</h1>
              <p className="text-gray-600 text-sm hidden md:block">
                Manage your product listings, inventory, and pricing all in one place. Keep your store updated to maximize sales.
              </p>
            </div>
          ...
          {sortedProducts.length > 0 ? (
              ...
              <table className="min-w-full bg-white text-sm">
                ...
                <tbody className="text-gray-700 divide-y divide-gray-200">
                  {sortedProducts.map((product: Product, idx: number) => {
                    const lowStock = isLowStock((product as any).stock ?? 0);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3 md:p-4">
                          <div className="w-12 h-12 md:w-16 md:h-16 bg-gray-100 flex items-center justify-center rounded-md overflow-hidden relative">
                            {product.images && product.images.length > 0 && !imgError ? (
                              <img src={product.images[0].url} alt={product.name} className="w-full h-full object-cover" onError={() => setImgError(true)} />
                            ) : product.images && product.images.length > 0 && imgError ? (
                              <span className="text-[10px] text-gray-500 text-center px-1 line-clamp-2">{product.name}</span>
                            ) : (
                        ...
                        <td className="p-3 md:p-4 font-medium max-w-[120px] md:max-w-none truncate">
                          ...
                          <div className="md:hidden text-xs text-gray-500 mt-1">{product.category?.name || "-"}</div>
                          <div className="sm:hidden text-xs mt-1">
                            <span
                              className={`px-2 py-1 rounded-full font-medium ${
                                (product as any).stock > 0 ? (lowStock ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800") : "bg-red-100 text-red-800"
                              }`}
                            >
                              {(product as any).stock > 0 ? (lowStock ? `Low stock (${(product as any).stock})` : `${(product as any).stock} in stock`) : "Out of stock"}
                            </span>
                          </div>
                          <div className="md:hidden text-xs mt-1">
                            <span className={`px-2 py-1 rounded-full font-medium ${((product as any).isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800")}`}>
                              {(product as any).isActive ? "Active" : "Inactive"}
                            </span>
                          </div>
                        </td>
                        <td className="p-3 md:p-4 hidden md:table-cell">{new Date(product.createdAt || "").toLocaleDateString()}</td>
                        ...
                        <td className="p-3 md:p-4 hidden sm:table-cell">
                          <span className={`px-2.5 py-1.5 rounded-full text-xs font-medium ${(product as any).stock > 0 ? (lowStock ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800") : "bg-red-100 text-red-800"}`}>
                            {(product as any).stock > 0 ? (lowStock ? `Low stock (${(product as any).stock})` : `${(product as any).stock} in stock`) : "Out of stock"}
                          </span>
                        </td>
                        <td className="p-3 md:p-4 hidden md:table-cell">
                          <span className={`px-2.5 py-1.5 rounded-full text-xs font-medium ${((product as any).isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800")}`}>
                            {(product as any).isActive ? "Active" : "Inactive"}
                          </span>
                        </td>
                        <td className="p-3 md:p-4">
            ...
            <div className="text-center py-12 md:py-16 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <p className="text-gray-500 mb-4 text-sm md:text-base px-2">Try adjusting your search or filter criteria to find what you&apos;re looking for.</p>
              {activeFilterCount > 0 && (
                <button onClick={clearAllFilters} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-sm">
                  Clear all filters
                </button>
              )}
            </div>
