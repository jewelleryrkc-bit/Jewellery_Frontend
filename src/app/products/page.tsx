...
export default function ProductsPage() {
  ...
  const category =   categoryParam && categoryParam.length > 0
    ? categoryParam.split(",")
    : undefined;
  ...
  // SEO logic for client-rendered title/meta tags with next/head
  let title = "Our Collection";
  let descArr: string[] = [];
  if (searchQuery) {
    title = `Search results for "${searchQuery}"`;
    descArr.push(`for "${searchQuery}"`);
  }
  if (category) descArr.push(`in category "${category}"`);
  if (material) descArr.push(`with material "${material}"`);
  if (minPrice || maxPrice)
    descArr.push(
      `price${minPrice ? ` from ${minPrice}` : ""}${maxPrice ? ` to ${maxPrice}` : ""}`
    );
  ...
  return (
    <div className="bg-gray-50">
      <SEO 
        ...
        description={searchQuery ? `Search results for "${searchQuery}"` : "Browse our jewelry collection"}
        products={products}
