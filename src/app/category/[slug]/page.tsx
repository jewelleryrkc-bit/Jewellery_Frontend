...
import { CATEGORY_BY_SLUG, GET_PRODUCTS_BY_CATEGORY, GET_WISHLISTS } from "../../../graphql/queries";
...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: categoryData, loading: categoryLoading, error: categoryError } = useQuery(
    CATEGORY_BY_SLUG,
    {
      variables: { slug }, // Use the unwrapped slug
  ...
  if (categoryLoading) return <div><LoadingPage/></div>;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-xl font-light text-gray-800 mb-6 tracking-tight text-center">{category.name}</h1>
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center mb-1 sm:mb-2">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => {
                    const isFilled = star <= Math.floor(product.averageRating || 0);
                    const isPartiallyFilled = !isFilled && star - 1 < (product.averageRating || 0);
                    
                    return (
                      <div key={star} className="relative">
                        <StarIcon
                          className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-gray-200"
                        />
                        {isFilled && (
                          <StarIcon
                            className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-yellow-400 absolute top-0 left-0 fill-yellow-400"
                          />
                        )}
                        {isPartiallyFilled && (
                          <div 
                            className="absolute top-0 left-0 overflow-hidden"
                            style={{ width: `${((product.averageRating || 0) - (star - 1)) * 100}%` }}
                          >
                            <StarIcon
                              className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-yellow-400 fill-yellow-400"
                            />
                          </div>
                        )}
                      </div>
